...
class World:
    ...
    def update_block_screen(self, drill_col, drill_row):
        ...
        accepted_y_start = math.floor(drill_row) - HEIGHT / BLOCK_SIZE

        for i in range(accepted_x_start, accepted_x_start + math.floor(WIDTH / BLOCK_SIZE), 1):
            # domain restriction is wrong
            # if (i < 0):
            #    continue
            for j in range(accepted_y_start, accepted_y_start + math.floor(HEIGHT / BLOCK_SIZE), 1):
...
class Drill(ImageRectObject):
    def __init__(self, world):
        #                                   x        y   width height
        super().__init__(NO_COLOR, (CENTER_X, CENTER_Y, 50, 50), 0, 0, "drill_image_file")
